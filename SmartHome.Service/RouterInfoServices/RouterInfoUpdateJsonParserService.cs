...
namespace SmartHome.Service
{
  public class RouterInfoUpdateJsonParserService : IHomeUpdateJsonParserService<RouterInfo>
  {
    ...
    private RouterInfo UpdateSmartRouterInfo()
    {
      ...
      routerInfo = new CommonService(_unitOfWorkAsync).GetRouterInfoByMacAddressAndHomeId(macAddress, home.HomeId);

      if (routerInfo != null)
      {
        UpdateWebBrokerInfo(_homeJsonEntity.WebBrokerInfo[0], home);
        UpdateUserHomeLink(userInfo);
        return UpdateRouter(routerEntity, routerInfo, home);
      }
      return null;
    ...
    public RouterInfo UpdateRouter(RouterInfoEntity routerEntity, RouterInfo router, Home home)
    {
